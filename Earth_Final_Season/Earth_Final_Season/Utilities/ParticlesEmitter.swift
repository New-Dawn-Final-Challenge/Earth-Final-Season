    ...
    var image: UIImage {
        ...
        case let .circle(radius):
            ...
            return UIGraphicsImageRenderer(size: size).image { context in
                ...
                cgContext.setShadow(offset: .zero, blur: 10, color: UIColor.white.withAlphaComponent(0.8).cgColor)

                // Draw the circle
                cgContext.addPath(CGPath(ellipseIn: CGRect(origin: .zero, size: size), transform: nil))
                cgContext.fillPath()
            }
        }
    }
